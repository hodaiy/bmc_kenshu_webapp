...
app.post(
  ...
  (req, res, next) => {
    ...
    const lines = fs.readFileSync(USERS_FILE, 'utf8').trim().split('\n');
    const isDefined = lines.some((line) => {
      return user === line.split(':')[0];
    });
    if (isDefined)
      return res.status(401).json({ error: 'User name already taken' });

    const hash = md5(password);
    fs.appendFileSync(USERS_FILE, `${user}:${hash}\n`);
    next();
  },
  loginMiddleware
);
